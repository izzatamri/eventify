...
class ServiceBookingController extends AbstractController
{
    ...
    #[Route('/{id}/status/{status}', name: 'change_status', methods: ['POST'], requirements: ['id' => '\d+', 'status' => 'confirmed|completed|cancelled'])]
    public function changeStatus(Request $request, int $eventId, int $id, string $status): Response
    {
        $event = $this->getEventOrRedirect($eventId);
        if ($event === null) {
            return $this->redirectToRoute('app_my_events');
        ...
        if (!$this->isCsrfTokenValid('booking_status_' . $id, $token)) {
            $this->addFlash('error', 'Invalid security token.');
            return $this->redirectToRoute('app_service_booking_index', ['eventId' => $event->getId()]);
        }

        $labels = [
            ServiceBooking::STATUS_CONFIRMED => 'confirmée',
            ServiceBooking::STATUS_COMPLETED => 'terminée',
            ServiceBooking::STATUS_CANCELLED => 'annulée',
        ];

        $previousStatus = $booking->getStatus();
        $booking->setStatus($status);

        if ($previousStatus !== ServiceBooking::STATUS_COMPLETED && $status === ServiceBooking::STATUS_COMPLETED) {
            $provider = $booking->getServiceProvider();
            if ($provider !== null) {
                $this->providerScoringService->addCompletionBonus($provider);
            } else {
                $this->entityManager->flush();
            }
        } else {
        ...
        $this->addFlash('success', 'Réservation ' . ($labels[$status] ?? $status) . '.');
