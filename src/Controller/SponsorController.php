...
class SponsorController extends AbstractController
{
    public function __construct(
        ...
        private readonly SponsorMailerService $sponsorMailer,
    ...
    public function accept(Request $request, Event $event, int $sponsorId): Response
    {
        ...
        $pdfContent = $this->pdfGenerator->generateSponsorshipAgreement($sponsor);

        try {
            $sigRequest = $this->yousignService->createSignatureRequest(
                'Sponsorship Agreement — ' . $event->getTitle() . ' — ' . $sponsor->getName()
            );
            $sigRequestId = $sigRequest['id'];

            $document = $this->yousignService->uploadDocument(
                $sigRequestId,
                $pdfContent,
                'Sponsorship_Agreement.pdf'
            );
            $documentId = $document['id'];

            $nameParts = explode(' ', $sponsor->getContactPersonName() ?? $sponsor->getName(), 2);
            $firstName = $nameParts[0];
            $lastName = $nameParts[1] ?? $firstName;
            $signerEmail = $sponsor->getContactPersonEmail() ?? $sponsor->getEmail();

            $this->yousignService->addSigner($sigRequestId, $documentId, $firstName, $lastName, $signerEmail);
            $this->yousignService->activateSignatureRequest($sigRequestId);

            $this->addFlash('success', 'Sponsor accepted. E-signature request sent via Yousign.');
        } catch (\Throwable $e) {
            $this->logger->error('Yousign API error during sponsor acceptance: ' . $e->getMessage(), [
                'sponsor_id' => $sponsor->getId(),
                'event_id' => $event->getId(),
            ]);
            $this->addFlash('warning', 'Sponsor accepted, but the e-signature request could not be sent: ' . $e->getMessage());
        }

        try {
            $this->sponsorMailer->sendAcceptedWithAgreement($sponsor, $pdfContent);
        } catch (\Throwable $e) {
            $this->logger->error('Failed to send sponsor acceptance email: ' . $e->getMessage(), [
                'sponsor_id' => $sponsor->getId(),
            ]);
            $this->addFlash('warning', 'Sponsor accepted, but the confirmation email could not be sent.');
        }
